...
fetch_video:
	...
	bin/parallel -j10 'bin/fetch_webm {} && echo -n .' ::: KhanAcademyOffline/*/videos/*
...
DVDs:
	seq 5 | bin/parallel mkdir -p DVD{}
	...
	bin/parallel -j0 cp -rl KhanAcademyOffline ::: DVD*
	bin/parallel "find {} -name '*.webm' -o -name '*.pdf' | grep -vE -f {}/This_is_{}" ::: DVD* | bin/parallel -X rm
...
clean:
	...
	find . -name '*.webm' | bin/parallel -X rm --
